export const filterTextForAPI = (text: string): string => {
  // Regex-Pattern, um nur Prozentzeichen, Zahlen, Buchstaben und Bindestriche zuzulassen
  const pattern = /[^%\d\w-]/g;
  // Text filtern und zurückgeben
...
export const fetchOctoAI = async (text: string, VITE_OCTOAI_TOKEN: string) => {
  try {
    // Text filtern
    const filteredText = filterTextForAPI(text);
    ...
    const requestData = {
      messages: [
        {
          role: "user",
          content: `Summarize the given text as briefly as possible in English. Place SUMMARY_START before the summary starts and SUMMARY_END after it is finished. This is the given text to summarize: ${filteredText}`,
        },
      ],
